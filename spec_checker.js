...
module.exports["SpecChecker"] = class SpecChecker {
    ...
    checkToken(line, token) {
        ...
        while (this.Specs.length > 0 && (this.Specs[0].source === undefined || this.Specs[0].source === "")) {
        ...
        if (source !== spec.source) {
            this.showFailureOnly || console.error("spec mismatch: next token is |%s| but spec has |%s|", source, spec.source)
            return false
        }
    ...
    displayScope(scope, newScopes, oldScopes) {
        ...
        } else if (!_.includes(newScopes, scope) && _.includes(oldScopes, scope)) {
    ...
    getScopes(spec) {
        ...
        const specScopes = [...this.scopeStack, ...(spec.scopes || [])].map(removeScopeName)
        if (spec.scopesEnd) {
            for (const scope of spec.scopesEnd.reverse()) {
                if (this.scopeStack[this.scopeStack.length - 1] !== scope) {
                    console.error("attempted to pop %s off scope stack, top of stack is %s", scope, this.scopeStack[this.scopeStack.length - 1])
                }
                this.scopeStack.pop()
            }
        }
