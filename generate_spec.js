...
module.exports = async function generateSpec(path, fixture) {
    ...
    for (let index = 0; index < spec.length; index += 1) {
        ...
        if (index < spec.length - 1) {
            ...
            let common = []
            for (let commonIndex = 0; commonIndex < Math.min(object.scopes.length, next.scopes.length); commonIndex += 1) {
            ...
            if (common.length > scopeStack.length) {
                object.scopesBegin = []
                for (let index = scopeStack.length; index < common.length; index += 1) {
            ...
            if (scopesEnd.length) {
                ...
                for (let scope of scopesEnd.slice().reverse()) {
                    if (scope === scopeStack[scopeStack.length - 1]) {
                        ...
                        console.error("Expected top of scope stack to be %s, but found %s", scope, scopeStack[scopeStack.length - 1])
                ...
                object.scopes = [...scopeStack, ...object.scopes.slice(nonLocalScopes.length)]
            }
        }
